...
export function getTypeDescription(id: number | string, fileName: string): any {
  const types = require(`../data/${fileName}.json`);
  return types[id];
}

export function getEventTypeDescription(name: string, fileName: string): any {
  const types = require(`../event-data/${fileName}.json`);
  return types[name];
}
...
export function decodeEvents(events: any[], contract: ContractPromise, fileName: string): any[] {
  return events
    .filter((record: any) => {
      ...
      return (
        event.method == 'ContractEmitted' && address.toString() === contract.address.toString()
      );
    })
    .map((record: any) => {
      const [address, data] = record.event.data;
